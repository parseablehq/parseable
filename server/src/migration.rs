...
use crate::{
    ...
    storage::{
        ...
        PARSEABLE_ROOT_DIRECTORY, SCHEMA_FILE_NAME, STREAM_METADATA_FILE_NAME,
...
async fn migration_stream(stream: &str, storage: &dyn ObjectStorage) -> anyhow::Result<()> {
    ...
    if stream_meta_found {
        ...
        let bytes = storage
            ...
            .await?;
        stream_metadata_value = serde_json::from_slice(&bytes).expect("stream.json is valid json");
        storage
            .put_object(&path, to_bytes(&stream_metadata_value))
            .await?;
    }
    let meta: ObjectStoreFormat =
        serde_json::from_slice(&serde_json::to_vec(&stream_metadata_value).unwrap()).unwrap(); //load stream metadata from storage
