    ...
    match stream_exists(&s3_client, &stream_name) {
        Ok(schema) => {
            ...
            // Proceed with validating against current schema and adding event to record batch. 
                ...
                let str_inferred_schema = infer_schema(body);
                if schema != str_inferred_schema.as_bytes() {
                    return  HttpResponse::Ok().body(format!("Invalid schema for {} ", stream_name))
                }
...
fn infer_schema(body: web::Json<serde_json::Value>) -> std::string::String {
