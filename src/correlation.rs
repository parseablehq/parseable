...
impl Correlations {
    ...
    pub async fn delete_for_user(&self, user_id: &str) {
        let mut correlations = self.0.write().await;
        // retaining all correlations except for user_id
        correlations.retain(|_id, corr_config| corr_config.user_id.as_str() != user_id);
    }
}
